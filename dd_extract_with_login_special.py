...
def getStat(vmid):
    ...
    burp0_headers = {"Connection": "close", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Safari/537.36 Edg/89.0.774.63", "DNT": "1", "Accept": "*/*", "Sec-Fetch-Site": "same-site",
...
def getFollowings(vmid, count):
    ...
    for i in range(1, math.ceil(count/50)+1):
        ...
        burp0_url = f"https://api.bilibili.com/x/relation/tag?mid={vmid}&tagid=-10&pn={i}&ps=50&jsonp=jsonp"
        response = session.get(burp0_url, headers=burp0_headers)
        # l += response.json().get('data').get('list')
        l += response.json().get('data')
    return l


def getInfo(mid):
    burp0_url = f"https://api.bilibili.com:443/x/space/acc/info?mid={mid}&jsonp=jsonp"
    burp0_headers = {"Connection": "close", "Accept": "application/json, text/plain, */*", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Safari/537.36 Edg/89.0.774.63", "DNT": "1", "Origin": "https://space.bilibili.com",
...
def getTags():
    burp0_url = "https://api.bilibili.com/x/relation/tags?jsonp=jsonp"
    burp0_headers = {"Connection": "close", "Accept": "application/json, text/plain, */*", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Safari/537.36 Edg/89.0.774.63", "DNT": "1", "Origin": "https://space.bilibili.com",
                     "Sec-Fetch-Site": "same-site", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Dest": "empty", "Referer": "https://space.bilibili.com/", "Accept-Encoding": "gzip, deflate", "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6"}
    response = session.get(burp0_url)
    jdata = response.json().get('data')
    tags = {}
    print(jdata)
    for tag in jdata:
        tags[tag['tagid']] = tag['count']
    return tags


def getMedalCount():
    burp0_url = "https://api.live.bilibili.com:443/live_user/v1/UserInfo/live_info"
    burp0_headers = {"Connection": "close", "Pragma": "no-cache", "Cache-Control": "no-cache", "Accept": "application/json, text/plain, */*", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Safari/537.36 Edg/89.0.774.54", "DNT": "1",
                     "Origin": "https://link.bilibili.com", "Sec-Fetch-Site": "same-site", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Dest": "empty", "Referer": "https://link.bilibili.com/p/center/index", "Accept-Encoding": "gzip, deflate", "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6"}
    response = session.get(burp0_url, headers=burp0_headers)
    count = response.json().get('data').get('count')
    return count.get('fansMedal')


def getFansMedal(count):
    l = []
    burp0_headers = {"Connection": "close", "Pragma": "no-cache", "Cache-Control": "no-cache", "Accept": "application/json, text/plain, */*", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.90 Safari/537.36 Edg/89.0.774.54", "DNT": "1",
                     "Origin": "https://link.bilibili.com", "Sec-Fetch-Site": "same-site", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Dest": "empty", "Referer": "https://link.bilibili.com/p/center/index", "Accept-Encoding": "gzip, deflate", "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6"}
    for i in range(1, math.ceil(count/10)+1):
        burp0_url = f"https://api.live.bilibili.com:443/fans_medal/v5/live_fans_medal/iApiMedal?page={i}&pageSize=10"
        response = session.get(burp0_url, headers=burp0_headers)
        l+=response.json().get('data').get('fansMedalList')
    return l
...
if __name__ == "__main__":
    ...
    print(f"[+] 已获取粉丝勋章个数：{medalCount}")
    medalList = getFansMedal(medalCount)
    for medal in medalList:
        mids.add(medal.get('target_id'))

    print(mids)
    print(f"[+] 已获取特别关注和已拥有粉丝勋章的 {len(mids)} 个up主")
    ...
    for mid in mids:
        ...
        roomid = user.get('live_room').get('roomid')
        uname = user.get('name')
